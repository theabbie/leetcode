...
for c in range(26):
    curr = [1 if cc == c else 0 for cc in s]
    ctr.append(LazySegTree(curr))
...
for _ in range(q):
    curr = input().split()
    if curr[0] == "1":
        l, r, c = int(curr[1]), int(curr[2]), curr[3]
        c = ord(c) - ord('a')
        for cc in range(26):
            if cc == c:
                ctr[cc].add(l - 1, r, (0, 1))
            else:
                ctr[cc].add(l - 1, r, (0, 0))
    if curr[0] == "2":
        l, r = int(curr[1]), int(curr[2])
        odds = 0
        for c in range(26):
            if ctr[c].query(l - 1, r) & 1:
                odds += 1
                if odds > 1:
                    break
